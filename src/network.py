...
class DenseNetwork():
    ...
    def set_loss_function(self, loss_function):
        ...
        # input type is assumed to be either str (loss function id) or 'LossFunction'
        if isinstance(loss_function, str):
            self.loss_function = get_lossfunction(loss_function)
    ...
    def propagate(self, intensor, return_ext=False):
        ...
        if not (intensor.is_vector() and intensor.shape[0]==self.input_dim):
            msg = 'Tensor with shape {intensor.shape} is invalid input'
            msg += ' for DenseNetwork with input dimension {self.input_dim}'
            raise Exception(msg)
        outtensors = []
    ...
    def fit(self, X_train, labels, validation_fraction=0.1):
        ...
        if( len(X_train.shape)!=2 ):
            raise Exception('X_train has unexpected shape: {}'.format(X_train.shape))
        if( len(labels.shape)!=1 ):
            raise Exception('Labels have unexpected shape: {}'.format(labels.shape))
        if( X_train.shape[0]!=len(labels) ):
            raise Exception('Number of instances in X_train and labels must be equal.')
        if( len(self.layers)==0 ):
            raise Exception('This network has no layers yet.')
        if( X_train.shape[1]!=self.input_dim ):
            raise Exception('X_train has {} features, but network has input size {}'
                            .format(X_train.shape[1],self.input_dim))
        if( self.nepochs==None ):
            raise Exception('Number of epochs was not yet set for this network.')
        if( self.optimizer==None ):
            raise Exception('Optimizer was not yet set for this network.')
        if( self.batch_size==None ):
            raise Exception('Batch size was not yet set for this network.')
        if( self.loss_function==None ):
            raise Exception('Loss function was not yet set for this network.')
    ...
    def predict(self, X_test):
        ...
        if( len(X_test.shape)!=2 ):
            raise Exception('X_test has unexpected shape: {}'.format(X_test.shape))
        if( len(self.layers)==0 ):
            raise Exception('This network has no layers yet.')
